...
SELECT Courses.CourseName,  
       MAX(Courses.Payment) AS [Цена],  
       COUNT(*) AS [Колличество]
FROM Groups INNER JOIN Courses
    ON Groups.GroupID = Courses.CourseID 
WHERE Courses.Payment >= 5000
...
SELECT 
	...
	SUM(c.Hours) AS [Всего часов],
	round(avg(cast(c.Hours as float(4))),2) AS[ROUND CAST]
FROM 
    TIMETABLE t
INNER JOIN 
...
WHERE 
    c.Hours > 50
GROUP BY 
    g.Department,c.CourseName,  c.Hours;
...
-- 7 Определить дни недели с наибольшим количеством занятий (используем HAVING)
SELECT 
...
GROUP BY 
    DayOfWeek
HAVING 
    COUNT(*) > 1 --фильтрация по сформированным группам
ORDER BY 
    COUNT(*) DESC;
